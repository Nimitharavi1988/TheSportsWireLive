...
- If the facts above contain no real, substantive, reportable information at all (e.g. just a bare name/place with no actual event, statement, or development — nothing a reader would learn from), do not write vague placeholder sentences like "X has been highlighted in recent coverage" or "no further details were provided." Return an empty commentary instead — an honest empty result is always better than a sentence that sounds like news but says nothing.
