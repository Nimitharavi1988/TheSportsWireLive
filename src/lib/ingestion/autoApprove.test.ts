...
describe("isAutoApprovable", () => {
  ...
  it("approves an article with a real substantive body and a real image", () => {
    expect(isAutoApprovable({ body: realBody, ...realImage })).toBe(true);
  });

  it("rejects an article with no body at all", () => {
    expect(isAutoApprovable({ body: null, ...realImage })).toBe(false);
  });

  it("rejects an article whose body is too short (a bare template, not real commentary)", () => {
    expect(isAutoApprovable({ body: thinBody, ...realImage })).toBe(false);
  });

  it("rejects an article with a real body but only a generic stock photo", () => {
    expect(isAutoApprovable({ body: realBody, ...stockImage })).toBe(false);
  });

  it("rejects an article with neither a real body nor a real image", () => {
    expect(isAutoApprovable({ body: null, ...stockImage })).toBe(false);
  });

  it("counts a crest-based match article with a real body as approvable", () => {
    expect(isAutoApprovable({ body: realBody, heroImageUrl: null, homeCrestUrl: "https://crests.football-data.org/1.png" })).toBe(true);
  });
});
