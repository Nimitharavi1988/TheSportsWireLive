...
- **Per-sport data reality** (as of 2026-09-15, verified live against each source):
  - `location`/venue: real for **cricket** (CricketData.org's `match.venue`) and **NFL** (ESPN's `venue.fullName`/`address`). **Not available for football** (football-data.org doesn't return a venue field on our API tier — confirmed via a live API call, not just docs, which can be stale/misleading for a different tier). Stored on `Article.venue`.
