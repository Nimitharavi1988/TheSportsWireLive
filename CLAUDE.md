...
- **Per-sport data reality** (as of 2026-09-15, verified live against each source):
  - `location`/venue: real for **cricket** (CricketData.org's `match.venue`), **NFL, NHL, and the domestic football leagues** (Bundesliga/Serie A/Ligue 1/MLS/Indian Super League — all from ESPN's `site.api.espn.com` scoreboard API, which provides `venue.fullName`/`address`). **Not available** for football-data.org's own coverage (doesn't return a venue field on our API tier — confirmed via a live API call, not just docs) or **volleyball** (API-Sports.io's `/games` response has no venue field). Stored on `Article.venue`.
